...
export default async function Terms() {
  const page = await client.fetch(`*[_type == "staticPage" && slug.current == "terms"][0]{title,intro,sections[]{number,title,body}}`)

  const title = page?.title ?? 'Terms of Use'
  const intro = page?.intro ?? 'DEWN LLC ("DEWN," "we," "us," "our").'
