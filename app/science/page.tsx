...
export default function Science() {
  ...
  useEffect(() => {
    client.fetch(`*[_type == "technicalThesis"][0]{
  ...
  const title = thesis?.pageTitle
  const subtitle = thesis?.pageSubtitle
  const disclaimer = thesis?.disclaimer
  ...
  const mergedSections = useMemo(() => {
    const sanityByNum = new Map((sections || []).map((s: any) => [String(s.number), s]));
